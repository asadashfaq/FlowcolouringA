...
# Nodes object containing 83 regions, alphas=.7, gammas=1
def EU_Nodes_regions(load_filename=None, full_load=False):
    return au.Nodes(admat='./settings/regionadmat.txt', path='./data/regions/', prefix = "ISET_region_", files=regions, load_filename=load_filename, full_load=full_load, alphas=np.ones(len(regions))*.7, gammas=np.ones(len(regions)))
