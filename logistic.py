...
def multi_TC(N, mode):  # N must be N = transcaps(EU_Nodes())
    ...
    if mode == 'linear':
        ...
        plt.axis([2016, 2050, 0, 3.5])
    elif mode == 'square':
        plt.yticks([1, 2, 3, 4, 5])
        plt.axis([2016, 2050, 0, 5])
    plt.xticks([2016, 2020, 2025, 2030, 2035, 2040, 2045, 2050])
...
def multi_GS(N):  # N must be N = transcaps(EU_Nodes())
    ...
    ax = plt.subplot(1, 1, 1)
    for n in N:
        plt.plot(years[1:], n.g_share[1:], alpha=0.15)
    plt.plot(years[1:], EU_T[1:], color=auplot.blue, lw=3, label='Europe')
    plt.plot(years[1:], BA_T[1:], color=auplot.orange, lw=3, label='Balkans')
    plt.plot(years[1:], WE_T[1:], color=auplot.green, lw=3, label='Western Europe')
...
def capPlotter(mode, direction):
    ...
    ax = plt.subplot(1, 1, 1)
    for node in range(nodes):
        plt.plot(years[1:], nodeCaps[node][1:], alpha=.15)
    plt.plot(years[1:], EU[1:], color=auplot.blue, lw=3, label='avg. Europe')
    plt.plot(years[1:], BA[1:], color=auplot.orange, lw=3, label='Balkans')
    plt.plot(years[1:], WE[1:], color=auplot.green, lw=3, label='Western Europe')
    plt.plot(years[1:], nodeCaps[18][1:], color=auplot.yellow, lw=3, label='Germany')
    plt.plot(years[1:], nodeCaps[21][1:], color=auplot.red, lw=3, label='Denmark')
    plt.plot(years[1:], nodeCaps[10][1:], color=auplot.lightblue, lw=3, label='Greece')
